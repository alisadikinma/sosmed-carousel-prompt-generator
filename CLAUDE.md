...
- **Creative Clarification Flow**: Every new carousel starts with 2-step creative clarification — Step 4b: agent presents 3 hook category options (PRIMARY/SECONDARY/WILDCARD with sample headline + vibe), user picks or provides custom. Step 4c: agent presents 3 visual direction combos (action + prop + costume + lighting/camera), user picks or modifies. Confirmed choices drive all downstream generation. Data sourced from hook-science.md mapping + hook-visual-library.md
