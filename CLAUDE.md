...
- **Watermark**: @handle watermark **center of image, faint barely-visible opacity** on all slides. For split-panel/comparison: watermark **on the vertical divider line**
...
| @handle watermark | **Center of image** (faint, barely-visible). On **vertical divider** for split-panel | YES |
...
| Watermark opacity | Faint, barely-visible (described as words in prompt, not percentage) |
...
| Watermark wrong position | Center of image (faint, barely-visible). Vertical divider for split-panel |
...
| "30%" rendered in image | Raw percentages render as text. Use descriptive words: "faint, barely-visible, very low opacity" |
