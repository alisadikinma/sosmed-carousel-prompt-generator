...
- **Default Language**: Bilingual — English headline (main) + Bahasa Indonesia subtitle. Captions in English. Single language on user request
...
- **Bahasa Subtitle**: Bahasa Indonesia translation of English headline, rendered below main headline in **accent color** (not white). Creates visual hierarchy and bilingual accessibility
...
- **Content Language**: Bilingual by default — English headline + Bahasa Indonesia subtitle. Single language (English-only, Bahasa-only, or other) on user request
...
| Bahasa subtitle | Below main headline — **accent color** (never white) | YES |
| Accent keywords (2-4) | Within headline — **accent color** | YES |
| Page number | **Top-left corner** — "[N]/[TOTAL]" small white | YES |
| Subject brand | In scene (logo/UI of discussed company) | When discussing specific brand |
| SWIPE CTA | Bottom center — "SWIPE (GESER) >" | All except CTA |
...
| Default language | Bilingual (EN headline + ID subtitle). Captions: English |
| Captions | All 4 platforms by default |
| Font weight | Extra Bold / Black weight (billboard-scale) |
| Brand icon position | Center of image, above watermark (thirty percent opacity) |
| Watermark position | Center of image, below brand icon (vertical divider for comparison split-panel) |
| Brand icon + watermark opacity | Both thirty percent opacity (spell out in prompt body — NEVER "30%") |
| Gradient zone | Bottom half (bottom third for thumbnails) |
| Page number | "[N]/[TOTAL]" top-left corner, all slides |
...
| Bahasa subtitle same color as headline | Bahasa subtitle must be accent color, never white. Check template for subtitle line |
...
| Wrong language | Default is bilingual (English headline + Bahasa subtitle). Captions in English. Check if user requested single language |
